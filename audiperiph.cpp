...
void Gammatone::startProcess(vector<double>::const_iterator inputData, size_t sizeOfData, std::vector<double>& output)
{
    window_size_ = sizeOfData;
    SignalBank localOutput;
    localOutput.init(num_channels_, window_size_, sample_rate_);
    for (int i = 0; i < num_channels_; ++i)
    {
        localOutput.set_centre_frequency(i, centre_frequencies_[i]);
    }
    ...
    process(inputData, localOutput);
    output = std::move(localOutput.getpowerAll());
}

void Gammatone::process(vector<double>::const_iterator inputData, SignalBank& output)
{
